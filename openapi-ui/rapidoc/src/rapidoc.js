...
export default class RapiDoc extends LitElement {
  ...
  static get properties() {
    return {
      ...
      endpointBodyLayout: { type: String, attribute: 'endpoint-body-layout' },
  ...
  connectedCallback() {
    ...
    if (!this.usePathInNavBar || !'path, summary,'.includes(`${this.usePathInNavBar},`)) { this.usePathInNavBar = 'path,summary'; }
  ...
  onClearSearch() {
    ...
    const searchEl2 = this.shadowRoot.getElementById('header-search');
    if (searchEl2) {
      searchEl2.value = '';
    }
    this.matchPaths = '';
