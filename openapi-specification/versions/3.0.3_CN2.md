...
OAS中的原始数据类型是基于JSON Schema Specification Wright Draft 00所支持的类型。请注意，整数作为一种类型也被支持，并且被定义为没有分数或指数部分的JSON数字。null不被支持为一种类型（参见nullable以获得替代解决方案）。模型使用Schema对象来定义，它是JSON Schema Specification Wright Draft 00的扩展子集。

基元有一个可选的修饰符属性：格式。OAS 使用几种已知的格式来详细定义正在使用的数据类型。然而，为了支持文档需求，格式属性是一个开放的字符串值属性，可以有任何值。格式如 "email"、"uuid "等，即使本规范没有定义，也可以使用。没有附带格式属性的类型遵循JSON Schema中的类型定义。不识别特定格式的工具可以默认回到类型，就像没有指定格式一样。
...
在整个规范中，描述字段都被注明为支持CommonMark标记格式。当OpenAPI工具渲染丰富的文本时，它必须至少支持CommonMark 0.27所描述的markdown语法。工具可以选择忽略一些CommonMark的功能来解决安全问题。
...
除非另有规定，否则所有作为 URL 的属性都可以是 RFC3986 定义的相对引用。相对引用使用服务器对象中定义的URL作为基础URI进行解析。

在$ref中使用的相对引用按照JSON Reference处理，使用当前文档的URL作为基础URI。也请参见Reference对象。
...
在下面的描述中，如果一个字段没有明确的REQUIRED或用MUST或SHALL来描述，它可以被认为是OPTIONAL。
...
REQUIRED（必需）。这个字符串必须是OpenAPI文档所使用的OpenAPI规范版本的语义版本号。openapi字段应该被工具规范和客户端用来解释OpenAPI文档。这与 API info.version 字符串无关。
REQUIRED（必需）。提供关于API的元数据。元数据可由工具根据需要使用。
一个服务器对象的数组，它提供了与目标服务器的连接信息。如果没有提供 servers 属性，或者是一个空数组，则默认值是一个服务器对象，其 url 值为 /。

关于哪些安全机制可以在整个API中使用的声明。值列表中包括可以使用的其他安全需求对象。只有满足其中一个安全需求对象才能授权请求。个别操作可以覆盖这个定义。为了使安全机制成为可选的，可以在数组中包含一个空的安全需求（{}）。

规范所使用的标签清单，包括附加元数据。标记的顺序可以用来反映解析工具对它们的排序。并非所有操作对象使用的标记都必须被声明。未声明的标记可以随机组织，也可以根据工具的逻辑来组织。列表中的每个标签名称必须是唯一的。
...
该对象提供了关于API的元数据。如果需要的话，这些元数据可以被客户使用，也可以在编辑或文档生成工具中显示，以方便客户使用。
...
需要。目标主机的URL。这个URL支持服务器变量，并且可以是相对的，以表明主机的位置是相对于OpenAPI文档被服务的位置。当一个变量用{括号}命名时，将进行变量替换。

描述URL指定的主机的可选字符串。CommonMark 语法可用于富文本表示。

变量名和其值之间的映射。该值在服务器的URL模板中用于替换。
...
一个字符串值的枚举，如果替换选项来自于一个有限的集合，则要使用该值。数组不能为空。

REQUIRED. 用于替换的默认值，如果没有提供替代值，则必须发送。注意这种行为与Schema对象对默认值的处理不同，因为在这些情况下参数值是可选的。如果定义了枚举，那么这个值应该存在于枚举的值中。

服务器变量的可选描述。CommonMark语法可用于丰富的文本表示。
...
为OAS的不同方面保留了一组可重复使用的对象。在组件对象中定义的所有对象都不会对API产生影响，除非它们明确地从组件对象之外的属性中引用。
...
保存到各个端点及其操作的相对路径。该路径被附加到来自服务器对象的URL上，以便构建完整的URL。由于ACL限制，路径可能为空。
...
到单个端点的相对路径。字段名必须以斜线（/）开头。为了构建完整的URL，路径被附加到服务器对象的url字段的扩展URL上（没有相对URL解析）。允许路径模板化。当匹配URL时，具体的（非模板化的）路径将在其模板化的对应路径之前进行匹配。具有相同层次结构但不同模板名称的模板化路径必须不存在，因为它们是相同的。在匹配不明确的情况下，由工具来决定使用哪一个。
...
假设使用以下路径，如果使用具体的定义，/pets/mine，将首先匹配。
...
描述单个路径上可用的操作。由于ACL的限制，路径项目可以是空的。路径本身仍然会暴露给文档浏览者，但他们不会知道哪些操作和参数是可用的。
...
允许对该路径项进行外部定义，引用的结构必须是路径项对象的格式。被引用的结构必须是路径项对象的格式。如果路径项对象字段同时出现在定义的对象和引用的对象中，则行为未定义。
可选的字符串摘要，适用于该路径的所有操作。
可选的字符串描述，适用于该路径的所有操作。CommonMark 语法可以用来表示丰富的文本。
