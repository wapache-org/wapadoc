...
定义或描述 API 的文档（或文档集）。一个OpenAPI定义使用并符合OpenAPI规范。

##### <a name="pathTemplating"></a>Path Templating
Path templating refers to the usage of template expressions, delimited by curly braces ({}), to mark a section of a URL path as replaceable using path parameters.

路径模板化是指使用模板表达式，以大括号({})为分隔线，将URL路径中的一段路径标记为可替换的路径参数。
...
OpenAPI规范使用Semantic Versioning 2.0.0.0(semver)进行版本化，并遵循Semver规范。

semver的major.mini部分（例如3.0）将指定OAS功能集。通常情况下，.patch版本解决的是本文档中的错误，而不是功能集。支持OAS 3.0的工具应该与所有OAS 3.0.*版本兼容。补丁版本不应该被工具化所考虑，例如不区分 3.0.0.0 和 3.0.1。

OpenAPI 规范的每一个新的次要版本都应允许在同一主要版本中的任何 OpenAPI 文档被更新到新的规范版本，并与之前的任何次要版本的规范有效。这样的更新必须只需要改变 openapi 属性到新的次要版本。

例如，一个有效的 OpenAPI 3.0.2 文档，在将其 openapi 属性更改为 3.1.0 后，将成为一个有效的 OpenAPI 3.1.0 文档，在语义上与原始的 OpenAPI 3.0.2 文档相当。新的OpenAPI规范的次要版本必须编写以确保这种形式的向后兼容性。

一个与OAS 3.*.*兼容的OpenAPI文档包含一个必要的openapi字段，它指定了它所使用的OAS的语义版本。(OAS 2.0 文档包含一个名为 swagger 和值 "2.0 "的顶层版本字段)。
...
一个OpenAPI文档可以由一个单独的文档组成，也可以由用户自行决定分成多个相连的部分。在后一种情况下，必须在规范中使用$ref字段来引用这些部分，如下文中的JSON模式定义。
...
OAS中的原始数据类型是基于JSON模式规范Wright Draft 00所支持的类型。
注意，整数作为一种类型也被支持，并且被定义为JSON数字，没有分数或指数部分。
null不支持作为一种类型（见 nullable 的替代解决方案）。
...
OAS使用几种已知的格式来详细定义正在使用的数据类型。
但是，为了支持文档的需要，格式属性是一个开放的字符串值属性，可以有任何值。
诸如 "email"、"uuid "等格式，即使本规范没有定义，也可以使用。
没有附带格式属性的类型遵循JSON Schema中的类型定义。不识别特定格式的工具可以默认为只识别该类型，就像没有指定格式一样。
...
在整个规范中，描述字段都支持CommonMark markdown 格式。
当 OpenAPI 工具渲染富文本时，它必须至少支持 CommonMark 0.27 所描述的 Markdown 语法。工具可以选择忽略一些CommonMark功能以解决安全问题。
...
除非另有规定，所有的URL属性都可以是RFC3986所定义的相对引用。
相对引用使用服务器对象中定义的URL作为基础URI进行解析。

在$ref中使用的相对引用会按照JSON引用的方式处理，使用当前文档的URL作为基础URI。另请参阅参考对象。
