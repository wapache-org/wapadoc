...
import org.wapache.openapi.generator.examples.ExampleGenerator;
import org.wapache.openapi.generator.meta.FeatureSet;
import org.wapache.openapi.generator.meta.GeneratorMetadata;
import org.wapache.openapi.generator.meta.Stability;
import org.wapache.openapi.generator.meta.features.*;
import org.wapache.openapi.generator.meta.features.*;
...
import org.wapache.openapi.generator.templating.mustache.*;
import org.wapache.openapi.generator.templating.mustache.*;
import org.wapache.openapi.generator.utils.ModelUtils;
import org.wapache.openapi.generator.utils.OneOfImplementorAdditionalData;
...
import org.wapache.openapi.v3.models.parameters.*;
...
import org.wapache.openapi.v3.parser.util.SchemaTypeUtil;

import static org.wapache.openapi.generator.utils.OnceLogger.once;
import static org.wapache.openapi.generator.utils.StringUtils.*;
...
public class DefaultCodegen implements CodegenConfig {
    ...
    */
    ...
    protected List<CodegenDiscriminator.MappedModel> getOneOfAnyOfDescendants(String composedSchemaName, String discPropName, ComposedSchema c, OpenAPI openAPI) {
        ...
        List<CodegenDiscriminator.MappedModel> descendentSchemas = new ArrayList();
        for (List<Schema> schemaList : listOLists) {
            ...
            for (Schema sc : schemaList) {
                ...
                CodegenDiscriminator.MappedModel mm = new CodegenDiscriminator.MappedModel(modelName, toModelName(modelName));
                descendentSchemas.add(mm);
                Schema cs = ModelUtils.getSchema(openAPI, modelName);
                Map<String, Object> vendorExtensions = cs.getExtensions();
                if (vendorExtensions != null && !vendorExtensions.isEmpty() && vendorExtensions.containsKey("x-discriminator-value")) {
                    String xDiscriminatorValue = (String) vendorExtensions.get("x-discriminator-value");
                    mm = new CodegenDiscriminator.MappedModel(xDiscriminatorValue, toModelName(modelName));
                    descendentSchemas.add(mm);
                }
    ...
    protected List<CodegenDiscriminator.MappedModel> getAllOfDescendants(String thisSchemaName, OpenAPI openAPI) {
        ...
        while (true) {
            ...
            CodegenDiscriminator.MappedModel mm = new CodegenDiscriminator.MappedModel(currentSchemaName, toModelName(currentSchemaName));
            descendentSchemas.add(mm);
            Schema cs = schemas.get(currentSchemaName);
            Map<String, Object> vendorExtensions = cs.getExtensions();
            if (vendorExtensions != null && !vendorExtensions.isEmpty() && vendorExtensions.containsKey("x-discriminator-value")) {
                String xDiscriminatorValue = (String) vendorExtensions.get("x-discriminator-value");
                mm = new CodegenDiscriminator.MappedModel(xDiscriminatorValue, toModelName(currentSchemaName));
                descendentSchemas.add(mm);
            }
        }
        return descendentSchemas;
    ...
    protected CodegenDiscriminator createDiscriminator(String schemaName, Schema schema, OpenAPI openAPI) {
        ...
        List<CodegenDiscriminator.MappedModel> uniqueDescendants = new ArrayList();
        if (sourceDiscriminator.getMapping() != null && !sourceDiscriminator.getMapping().isEmpty()) {
            for (Entry<String, String> e : sourceDiscriminator.getMapping().entrySet()) {
                ...
                uniqueDescendants.add(new CodegenDiscriminator.MappedModel(e.getKey(), modelName));
            }
        }
        ...
        if (!this.getLegacyDiscriminatorBehavior() || legacyUseCase) {
            ...
            List<CodegenDiscriminator.MappedModel> otherDescendants = getAllOfDescendants(schemaName, openAPI);
        ...
        if (ModelUtils.isComposedSchema(schema) && !this.getLegacyDiscriminatorBehavior()) {
            List<CodegenDiscriminator.MappedModel> otherDescendants = getOneOfAnyOfDescendants(schemaName, discPropName, (ComposedSchema) schema, openAPI);
            for (CodegenDiscriminator.MappedModel otherDescendant : otherDescendants) {
                if (!uniqueDescendants.contains(otherDescendant)) {
                    uniqueDescendants.add(otherDescendant);
                }
